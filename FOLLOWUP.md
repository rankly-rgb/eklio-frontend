...
| 8b | ⚠ **BLOQUANT — vérifier que `credit_ledger` gagne bien trente lignes par mois généré** (F25). Le chemin Batch ne réservait AUCUN crédit : le quota de trente `post_generation` n'était appliqué qu'au premier mois d'un compte, jamais aux suivants. Une ligne de SQL le dit : `select date_trunc('month', created_at), count(*) from credit_ledger group by 1;`. ⚠ **Avant l'étape 9**, sinon on ouvre la facturation sur un compteur qui ne compte pas. | agent |
